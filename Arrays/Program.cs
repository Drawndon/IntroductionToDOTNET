...
namespace Arrays
{
	internal class Program
	{
		static void Main(string[] args)
		{
			...
			Random rand = new Random(0); //Random(0) задаем точку отсчета seed
			...
			#region FOREACH
			//foreach(int i in arr) не дает менять значения
			//{
			//	i = rand.Next(100);
			//}
			...
			Console.WriteLine($"Среднее-арифметическое лементов массива: {(double)sum / count}");
			int min, max;
			min = max = arr[0][0];
			foreach (int[] i in arr)
			{
